...
export default class Sprite {
	...
	render(canvasX, canvasY, scale = { x: 1, y: 1 }) {
		const img = this.graphic.image;
		// Guard against broken or not-yet-loaded images to prevent InvalidStateError
		if (img && img.complete && img.naturalWidth > 0 && img.naturalHeight > 0) {
			...
			// Fallback: draw a placeholder rect so game continues rendering
			this.graphic.context.fillStyle = '#cc4444';
			this.graphic.context.fillRect(
				canvasX,
				canvasY,
				this.width * scale.x,
				this.height * scale.y,
